...
bootstrap_python_tooling() {
    ...
    if is_macos && [[ "$(which python3)" == *"/opt/homebrew/"* ]]; then
...
run_npm_audit() {
    ...
    if command -v jq >/dev/null 2>&1 && [ -f "$FRONTEND_AUDIT" ]; then
        ...
        SEVERITY_COUNTS[medium]=$moderate
...
scan_for_secrets() {
    ...
    for pattern in "${patterns[@]}"; do
        ...
        if [ -n "$matches" ]; then
            ...
            echo '```' >> "$SECRETS_SCAN"
            echo "$matches" | head -20 >> "$SECRETS_SCAN"
            echo '```' >> "$SECRETS_SCAN"
            echo "" >> "$SECRETS_SCAN"
            found=true
        fi
...
write_summary() {
    ...
    for severity in "${!SEVERITY_COUNTS[@]}"; do
        total_vulns=$((total_vulns + SEVERITY_COUNTS[$severity]))
    done
...
JSON
    ...
    log_info "  Critical: ${SEVERITY_COUNTS[critical]}"
    log_info "  High: ${SEVERITY_COUNTS[high]}"
    log_info "  Medium: ${SEVERITY_COUNTS[medium]}"
    log_info "  Low: ${SEVERITY_COUNTS[low]}"
    log_info "════════════════════════════════════════"
